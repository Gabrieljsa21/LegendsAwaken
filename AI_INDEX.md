...
<!-- Last updated: 2026-04-16 · Phase 3A.2 complete (bot layer) -->
...
### `BannerHistorico` / `BannerProgresso` — `Domain/Entities/Banner/`
...
| `GachaService` | Pull logic, pity, race distribution | `RealizarPullAsync(userId, bannerId, qtd)` |
| `BannerService` | Banner config and selection | |
| `BannerHistoricoService` | Pull history per user/banner | |
...
| `IBannerHistoricoRepository` | `BannerHistoricoRepository` | `Infrastructure/Repositories/BannerHistoricoRepository.cs` |
...
| `HeroisTags` | `HeroiTag` |
| `BannerHistorico` | `BannerHistorico` |
| `BannerProgressos` | `BannerProgresso` |
| `Parties` | `Party` |
...
### `CommandHandler.cs`
Handles `SlashCommandExecuted`, `ButtonExecuted`, `AutocompleteExecuted`, `SelectMenuExecuted`.  
Constructor injects: `HeroiService`, `GeracaoDeDadosService`, `BannerService`, `BannerHistoricoService`, `UsuarioService`, `GachaService`, `RacaService`, `AtributoBonusService`, `CombatService`, `PartyService`, `CidadeService`, `CraftingService`, `ArenaService`.
...
| `/invocar` | `InvocarCommand` | `Commands/InvocarCommand.cs` |
| `/banner` | `BannerCommand` | `Commands/BannerCommand.cs` |
...
| File | Purpose |
|---|---|
| `Infrastructure/Providers/BannerConfiguracoesProvider.cs` | Static `BannerPadrao` config; `TabelaRacas(chanceHumano)` auto-scales via `Enum.GetValues<Raca>()` |
| `Application/Helpers/BannerDinamico.cs` | Dynamic banner logic; `_todasRacas = Enum.GetValues<Raca>().ToList()` |
...
3. Nothing else — `BannerConfiguracoesProvider.TabelaRacas`, `BannerDinamico._todasRacas` auto-adapt
